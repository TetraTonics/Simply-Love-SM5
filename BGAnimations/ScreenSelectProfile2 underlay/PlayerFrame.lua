...
-- ----------------------------------------------
				-- "recent song" commented out for GoodReads8

				-- the song that was most recently played, presented as "group name/song name", eventually					-- the song that was most recently played, presented as "group name/song name", eventually
				-- truncated so it passes the "How to Cook Delicious Rice and the Effects of Eating Rice" test.					-- truncated so it passes the "How to Cook Delicious Rice and the Effects of Eating Rice" test.
				-- LoadFont("Common Normal")..{
				-- 	Name="MostRecentSong",
				...
				LoadFont("Common Normal")..{
					...
					SetCommand=function(self, params)
						if params then
							self:visible(true):settext(params.quote and ("~"..params.quote) or "")
						else
							self:visible(false):settext("")
						end
					end
