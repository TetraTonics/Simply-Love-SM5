...
local judgment_graphics = {
	...
	Tutorial=GetJudgmentGraphics("Tutorial"),
}
...
local GoodReadsCountry = function(name)
	...
	return GoodReads.Players[name].country
end

local GoodReadsQuote = function(name)
	if not GoodReads.Players[name] then return nil end
	return GoodReads.Players[name].quote
end

local GoodReadsExp = function(name)
	if not GoodReads.Players[name] then return nil end
	return GoodReads.Players[name].exp
end
...
for i=1, PROFILEMAN:GetNumLocalProfiles() do
	...
	local data = {
		...
		judgment = judgment,
		country = "PSU",
		quote = GoodReadsQuote(profile_name),
		exp = GoodReadsExp(profile_name)
	}
