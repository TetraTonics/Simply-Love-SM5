...
-- FIXME: making numCols and numRows configurable variables made sense when SSMTutorial was more grid-like
...
local PruneSongsFromGroup = function(group)
	...
	for i,song in ipairs(SONGMAN:GetSongsInGroup(group)) do
		...
		and song:MusicLengthSeconds() < PREFSMAN:GetPreference("LongVerSongSeconds") then
			-- ensure that at least one stepchart has a meter ≤ TutorialMaxMeter (10, by default)
			for steps in ivalues(song:GetStepsByStepsType(steps_type)) do
				if steps:GetMeter() <= ThemePrefs.Get("TutorialMaxMeter") then
					songs[#songs+1] = song
					break
				end
			end
...
---------------------------------------------------------------------------
-- parse ./Other/TutorialMode-Groups.txt to find which groups will appear in SSMTutorial
-- returns an indexed table of group names as strings

local GetGroups = function()
	local path = THEME:GetCurrentThemeDirectory() .. "Other/TutorialMode-Groups.txt"
...
---------------------------------------------------------------------------
-- parse ./Other/TutorialMode-DefaultSong.txt to find one or more songs to
-- default to when SSMTutorial first loads
-- returns a song object

local GetDefaultSong = function(groups)

	local path = THEME:GetCurrentThemeDirectory() .. "Other/TutorialMode-DefaultSong.txt"
...
-- GetGroups() will read from ./Other/TutorialMode-Groups.txt
...
-- if no current_song, check ./Other/TutorialMode-DefaultSong.txt
