...
local OptionRows = {
	{
		Name = "Charts",
		HelpText = THEME:GetString("ScreenSelectMusicTutorial", "SelectDifficulty"),
		...
		Values = function()
			...
			for chart in ivalues(SongUtil.GetPlayableSteps( GAMESTATE:GetCurrentSong() )) do
				if chart:GetMeter() <= ThemePrefs.Get("TutorialMaxMeter") then
					steps[#steps+1] = chart
				end
			end
			return steps
		...
		OnLoad=function(actor, pn, choices, values)
			...
			-- previous chart's meter.  I attempting to match by numerical Meter makes more sense for Tutorial mode than
	...
	{
		Name = "Speed",
		HelpText = THEME:GetString("ScreenSelectMusicTutorial", "SelectSpeedMod"),
		Choices = function()
			return {
				THEME:GetString("ScreenSelectMusicTutorial", "Normal"),
				THEME:GetString("ScreenSelectMusicTutorial", "MoreSpace"),
				THEME:GetString("ScreenSelectMusicTutorial", "LessSpace"),
			}
		end,
